...
plot_ellipse <- function(model_, id) {
    ...
    if (id[1] != 1 && id[2] != 1) {
        model <- lm.beta(model_)
        scale_ <- diag((model$standardized.coefficients / model$coefficients))
        confidenceEllipse(model, which.coef = id, vcov. = function(x) scale_ %*% vcov(x) %*% scale_)
        points(0, 0, col = "red", pch = 19)
        lines(x = c(0, coef(model)[id[1]]), c(0, coef(model)[id[2]]), col = "red")
        # confidenceEllipse(lm.beta(model_), which.coef = id, vcov. = function(x) cov2cor(vcov(x)))
        # points(0, 0, col = "red", pch = 19)
        # lines(x = c(0, coef(lm.beta(model_))[id[1]]), c(0, coef(lm.beta(model_))[id[2]]), col = "red")
    }
...
```{r}
# plot(model_manual)
ols_plot_resid_lev(model_manual_)
print(df_names[38, ])
```
...
```{r}
plot(sort(mahalanobis_distance(df %>% select(-CRIME))$mahal.dist, decreasing = TRUE))
...
## Новое
Попытка прогнозировать преступность в Петербурге
```{r}
...
```
Плохо согласуется потому что модель построена на американских городах по данным прошлого века.
Данные собирались по разному, законы разные...
